...
class PO(PyoObject):
    """
    Generic PyoObject (PO), which can be modified easily
    When modified, a copy is made and changes added, the original is deleted and replaced
    Modification occurs between generations of a genetic algorithm.
    Each individual in the GA is a PyoObject. The GP Tree is the creation rules for a PO.
...
def sources(sel, one, two):
    # sel between 1 and 5
    if sel == 1:
        # mul and add
        return BrownNoise([one, two]).out()
    if sel == 2:
        #
        return
